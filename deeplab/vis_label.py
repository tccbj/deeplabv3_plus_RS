...
"""
# modified at 2019/04/09
# add groundtruth visualize
...
from deeplab.datasets import data_generator_shuichan as data_generator
...
flags.DEFINE_enum('colormap_type', 'pascal', ['pascal', 'cityscapes','shuichan'],
...
# The format to save groundtruth\
_GT_FORMAT = '%06d_gt'
...
def _process_batch(sess, original_images, semantic_images, semantic_predictions, 
  ...
  (original_images, semantic_images, semantic_predictions) = sess.run([original_images, semantic_images, semantic_predictions])
  num_image = semantic_predictions.shape[0]
  for i in range(num_image):
    image_height = 321#np.squeeze(image_heights[i])
    image_width = 321#np.squeeze(image_widths[i])
    original_image = np.squeeze(original_images[i])
    semantic_image = np.squeeze(semantic_images[i])
    ...
    if FLAGS.also_save_raw_predictions:
      image_filename = _PREDICTION_FORMAT % (image_id_offset + i)#os.path.basename(image_names[i])
      ...
      with tf.train.MonitoredSession(
        ...
        while not sess.should_stop():
          ...
          _process_batch(sess=sess,
                         ...
                         semantic_images=samples[common.LABEL],
