...
"""
# 和原来的data_generator差别就是读图片数据不一样，用的tf.decode_raw，原来根据后缀用tf.image.decode_jpeg
# modified at 0410
# add another datasets *_fine ,trainval
...
# all in 321*321
_SHUICHAN = DatasetDescriptor(
    splits_to_sizes={
        'train': 1080,  # num of samples in images/training
        'val': 268,  # num of samples in images/validation
        'trainval': 1348   # train + val
        'test': 184317,  # num of samples in images/test
        'train_fine': 1822    # seg_fine train 740+1082
        'val_fine': 454      # seg_fine val 184+270
        'trainval_fine': 2276  # seg fine trainval 924+1352
    },
    num_classes=2,
    ignore_label=255,
)
...
class Dataset(object):
  ...
  def _parse_function(self, example_proto):
    ...
    height = 321#parsed_features['image/height']
    width = 321#parsed_features['image/width']
...
#    image = _decode_image(parsed_features['image/encoded'], channels=3)
    ...
    if self.split_name != common.TEST_SET:
        label = tf.decode_raw(parsed_features['image/segmentation/class/encoded'],tf.uint8)
